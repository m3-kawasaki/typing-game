...
## 概要
- **目的**: ブラウザで動く、シンプルで気持ちよく遊べる30秒タイピングゲームを提供する。
...
- **リーダーボード（常時）**: 画面下部に上位3件（「名前 — 完了語数, 精度%」）を常時表示。
...
- **サウンド**（WebAudioによる生成音）:
  - 成功音: 880Hz/40ms/サイン波（短い「ピッ」）
  - ミス音: 220Hz/120ms/スクエア波
  - 残り時間チック: 600Hz/40ms/サイン波（残5秒から1Hz）
  - ミュート切替・音量調整可。設定は保存。
  - ブロッキングUI（名前入力のprompt等）中でも、オシレータの停止時刻を予約するため音は所定時間で確実に止まる。
...
- **ハイスコア/ランキング**: 端末ローカル保存（`localStorage`）。
  - 表示: 結果ダイアログに上位3件（「名前 — 完了語数, 精度%」）を表示。
  - 常時表示: 画面下部にも上位3件を常時表示し、更新時に即時反映。
  - 登録: ラウンド終了時、今回のスコアが上位3に入った場合のみ名前入力ダイアログを表示（未入力は「名無しさん」）。
  - 並び順: 完了語数の降順 → 精度（%）降順 → WPM降順 → 古い記録優先。
  - 保存キー: `typingGame:scoresV1`（ランキング配列）、`typingGame:bestCompleted`（完了語数の自己ベスト）。

## タイマー・設定
- **制限時間**: 既定`30`秒（最小5〜最大120秒）。
...
- **演出**: 結果表示時にキャンバスオーバーレイで花吹雪（桜色の紙吹雪）を数秒間表示。
...
- 単語完了で成功演出と成功音が鳴る。
- 既定の制限時間は30秒で、設定から変更・保存できる。
- `assets/words.txt`差し替え、またはUIインポートで出題が切り替わる。
- 結果画面に完了単語数・精度・CPM/WPM・上位3ランキングが表示され、同ランキングは画面下部にも常時表示される。
...
- **音**: `AudioContext`の`OscillatorNode`と`GainNode`で生成音を再生（短いエンベロープで停止時刻を予約）。
